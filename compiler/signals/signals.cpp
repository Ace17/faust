...
{
  Tree x;
  return isTree(t, SIGINPUT, x) && isInt(x->node(), i);
}
...
{
  Tree x;
  return isTree(t, SIGOUTPUT, x, t0) && isInt(x->node(), i);
}
...
{
  Tree t;
  return isTree(s, SIGBINOP, t, x, y) && isInt(t->node(), op);
}
...
{
  Tree t, n, f;
  return isTree(s, SIGFCONST, t, n, f);
}
...
{
  Tree x;
  return isTree(t, SIGPROJ, x, rgroup) && isInt(x->node(), i);
}
...
{
  return tree(SIGWAVEFORM, wf);
}

bool isSigWaveform(Tree s)
{
  return isTree(s, SIGWAVEFORM);
}
...
{
  Tree lbl, params;
  return isTree(s, SIGHSLIDER, lbl, params);
}
...
{
  Tree lbl, params;
  return isTree(s, SIGVSLIDER, lbl, params);
}
...
{
  Tree lbl, params;
  return isTree(s, SIGNUMENTRY, lbl, params);
}
...
{
  Tree m;
  return isTree(s, SIGTUPLE, m, ls) && isInt(m->node(), mode);
}
