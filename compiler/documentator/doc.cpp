...
static void printfaustlisting(string& faustfile, ostream& docout)
{
  ...
  if(faustfile != "" && src.good())
  {
    while(getline(src, s))   /** We suppose there's only one <mdoc> tag per line. */
    {
      ...
      if(foundopendoc != string::npos && gStripDocSwitch)
        isInsideDoc = true;
      ...
      if(foundclosedoc != string::npos && gStripDocSwitch)
        isInsideDoc = false;
    }
  }
  else
...
static void prepareDocEqns(const vector<Tree>& docBoxes, vector<Lateq*>& docCompiledEqnsVector)
{
  ...
  if(!eqBoxes.empty())
  {
    ...
    mapEvalDocEqn(eqBoxes, gExpandedDefList, evalEqBoxes);    ///< step 1. Evaluate boxes.
    ...
    vector<Tree> eqSigs;
    mapPrepareEqSig(evalEqBoxes, eqInputs, eqOutputs, eqSigs);    ///< step 4&5. Propagate and prepare signals.
