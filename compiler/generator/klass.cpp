...
{
  tab(n, fout);
  fout << "static void metadata(Meta* m) \t{ ";
...
void Klass::printComputeMethod(int n, ostream& fout)
{
  ...
  else if(gVectorSwitch)
  {
    ...
    {
    case 0: printComputeMethodVectorFaster(n, fout);
      break;
    case 1: printComputeMethodVectorSimple(n, fout);
      break;
    default: cerr << "unknown loop variant " << gVectorLoopVariant << endl;
      exit(1);
    }
  }
  else
...
void Klass::printComputeMethodVectorFaster(int n, ostream& fout)
{
  ...
  // than gVecSize
  tab(n + 1, fout);
  fout << subst("virtual void compute (int fullcount, $0** input, $0** output) {", xfloat());
  printlines(n + 2, fZone1Code, fout);
  printlines(n + 2, fZone2Code, fout);
  printlines(n + 2, fZone2bCode, fout);
...
void Klass::printComputeMethodScheduler(int n, ostream& fout)
{
  ...
  tab(n + 3, fout);
  fout << "fFullCount = min (" << gVecSize << ", fullcount-fIndex);";
  tab(n + 3, fout);
  fout << "TaskQueue::Init();";
