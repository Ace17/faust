...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void buildUserInterface(UI* interface) {
		...
		interface->declare(&fslider1, "unit", "lin");
		interface->addHorizontalSlider("Gain", &fslider1, 1.0f, 0.0f, 8.0f, 0.01f);
		interface->addHorizontalSlider("Q", &fslider0, 1.0f, 0.01f, 1e+02f, 0.01f);
		interface->closeBox();
	}
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		float 	fSlow0 = max(0.001f, float(fslider0));
		float 	fSlow1 = sqrtf(max(1e-05f, float(fslider1)));
		...
		float 	fSlow8 = (0.5f * ((fSlow1 * fSlow3) / fSlow0));
