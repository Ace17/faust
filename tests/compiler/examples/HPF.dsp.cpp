...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		float 	fSlow5 = (1.0f / (1 + fSlow1));
		float 	fSlow6 = (1 + fSlow3);
		float 	fSlow7 = (0.5f * fSlow6);
		float 	fSlow8 = (0 - fSlow6);
		FAUSTFLOAT* input0 = input[0];
		FAUSTFLOAT* output0 = output[0];
		for (int i=0; i<count; i++) {
			fRec0[0] = ((float)input0[i] - (fSlow5 * ((fSlow4 * fRec0[1]) + (fSlow2 * fRec0[2]))));
			output0[i] = (FAUSTFLOAT)(fSlow5 * (((fSlow8 * fRec0[1]) + (fSlow7 * fRec0[0])) + (fSlow7 * fRec0[2])));
			// post processing
